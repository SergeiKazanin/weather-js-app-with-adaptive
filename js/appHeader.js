...
import { cToF, decimalAdjust, fToC, resetWeatherContent } from "./helper.js";
...
export const createHeader = (city) => {
    ...
    unitsC.addEventListener('click', () => {
        ...
        temperature.textContent = decimalAdjust('round',convertTemp,-1);
    })
    ...
    unitsF.addEventListener('click', () => {
        ...
        temperature.textContent = decimalAdjust('round',convertTemp,-1); //Math.round(convertTemp);
    })
    ...
    const showError = (messsge) => {
        errorBlock.classList.add('show-error');
        errorBlock.textContent = messsge;
    }
    ...
    window.addEventListener('click', (e) => {
        if (e.target == searchInput
            || e.target == searchBtn
            || e.target == cityChange
            || headerCity.firstElementChild == cityName) {
            return;
        } else {
            headerCity.innerHTML = '';
